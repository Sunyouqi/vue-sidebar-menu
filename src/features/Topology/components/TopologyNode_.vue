...
const leftPorts = computed(() => (props.data.interfaces ?? []).filter((interfaces) => interfaces.side === 'left'))
const rightPorts = computed(() => (props.data.interfaces ?? []).filter((interfaces) => interfaces.side === 'right'))
</script>
...
<template>
    <div class="device-node switch-node">
        ...
        <template v-for="(port, index) in leftPorts" :key="port.guid">
            <Handle :id="port.guid" type="target" :position="Position.Left" :style="portStyle(index)"
                connectable="single" @dblclick.stop="emit('port-dblclick', port)"
                @contextmenu.prevent.stop="emit('port-contextmenu', port, $event, props.id)" />
            <span class="port-label port-label-left" :style="portStyle(index)"
                @dblclick.stop="emit('port-dblclick', port)"
                @contextmenu.prevent.stop="emit('port-contextmenu', port, $event, props.id)">{{ port.name }}</span>
        </template>
        <template v-for="(port, index) in rightPorts" :key="port.guid">
            <Handle :id="port.guid" type="source" :position="Position.Right" :style="portStyle(index)"
                connectable="single" @dblclick.stop="emit('port-dblclick', port)"
                @contextmenu.prevent.stop="emit('port-contextmenu', port, $event, props.id)" />
            <span class="port-label port-label-right" :style="portStyle(index)"
                @dblclick.stop="emit('port-dblclick', port)"
                @contextmenu.prevent.stop="emit('port-contextmenu', port, $event, props.id)">{{ port.name }}</span>
        </template>
    </div>
